...
export const handleContribute = async (amount: string) => {
  ...
  try {
    ...
    if (accounts.length === 0) {
      throw new Error(
        "No connected accounts found. Please connect your wallet."
      );
    }
    ...
    const daosContract = new web3.eth.Contract(
      CONTRACT_ABI as any,
      contractAddress
    );
    console.log("Contract object created:", daosContract);

    const contributedAmountYetRaw = await daosContract.methods
      .contributions(contributor)
      .call();
    const contributedAmountYet = Number(contributedAmountYetRaw) / 10 ** 18;

    const userTiers = await daosContract.methods
      .getWhitelistInfo(contributor)
      .call();
    ...
    const tierLimit = await daosContract.methods
      .tierLimits(Number(tierNumber))
      .call();
    ...
    const currentAllowanceRaw = await tokenContract.methods
      .allowance(contributor, contractAddress)
      .call();
    ...
    if (currentAllowance < Number(weiAmount)) {
      ...
      const gasEstimate = await tokenContract.methods
        .approve(contractAddress, requiredApproval)
        .estimateGas({
          from: contributor,
        });
      const approveTx = {
    ...
    let gasEstimate;
    try {
      gasEstimate = await daosContract.methods
