      ...
      <Table className="p-4">
          ...
          {table.getHeaderGroups().map((headerGroup) => (
            <TableRow key={headerGroup.id}>
              {headerGroup.headers.map((header) => (
                <TableHead key={header.id} className="text-[#e4e6e7] h-16 text-sm font-normal font-['Work Sans'] tracking-wide">
        ...
        <TableBody>
          {table.getRowModel().rows.map((row) => (
            <TableRow
              key={row.id}
              className="border-b border-[#121212] last:border-b-0"
            >
        ...
        <div className="flex items-center space-x-2">
          {Array.from({ length: table.getPageCount() }, (_, i) => i + 1).map((page) => (
            <Button
              ...
              className={`w-6 h-8 p-1 ${table.getState().pagination.pageIndex === page - 1
